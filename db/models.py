...
class Company(Base, TimestampMixin, ReprMixin):
    ...
    logo_path = synonym("logo_file_path")
...
class LineListItem(Base, TimestampMixin, ReprMixin):
    ...
    ndt_extent_rt_pct = Column(Float, default=0)
    ndt_percent_rt = synonym("ndt_extent_rt_pct")
    ndt_extent_ut_pct = Column(Float, default=0)
    ndt_percent_ut = synonym("ndt_extent_ut_pct")
    ...
    ndt_extent_pt_pct = Column(Float, default=0)
    ndt_percent_pt = synonym("ndt_extent_pt_pct")
    ndt_extent_mt_pct = Column(Float, default=0)
    ndt_percent_mt = synonym("ndt_extent_mt_pct")
    insulation_thickness_mm = Column(Float)
    sour_service = Column(Boolean, default=False)
    size_dn = Column(String(30))
    dn = synonym("size_dn")
...
class MaterialTakeOff(Base, CreatedOnlyMixin, ReprMixin):
    ...
    two_year_quantity = Column(Float)
    two_year_qty = synonym("two_year_quantity")
    purchase_quantity = Column(Float)
    purchase_qty = synonym("purchase_quantity")
    ...
    miv_quantity = Column(Float)
    miv_qty = synonym("miv_quantity")
...
class MaterialIssueRecord(Base, CreatedOnlyMixin, ReprMixin):
    ...
    issued_qty = synonym("issued_quantity")
    quantity_issued = synonym("issued_quantity")
...
class Weld(Base, TimestampMixin, ReprMixin, SoftDeleteMixin):
    ...
    base_material = Column(String(100))
    material = synonym("base_material")
    schedule = Column(String(30))
    welding_process = Column(String(50))
    ...
    welder_stencil_number = Column(String(100), index=True)
    welder_id = synonym("welder_stencil_number")
    ...
    pwht_completed = Column(Boolean, default=False)
    pwht_done = synonym("pwht_completed")
    ...
    ndt_extent_rt_pct = Column(Float)
    ndt_percent_rt = synonym("ndt_extent_rt_pct")
    ndt_extent_pt_pct = Column(Float)
    ndt_percent_pt = synonym("ndt_extent_pt_pct")
    pwht_required = Column(Boolean, default=False)
    ...
    left_quantity = Column(Float)
    left_qty = synonym("left_quantity")
    right_component = Column(String(150))
    right_quantity = Column(Float)
    right_qty = synonym("right_quantity")
    is_on_hold = Column(Boolean, default=False, index=True)
    hold = synonym("is_on_hold")
    ...
    root_welder_id = synonym("welder_stencil_number")
    cap_welder_id = synonym("welder_stencil_number")
    shop_field = synonym("weld_type")
    ag_ug = synonym("install_location")
    base_metal = synonym("base_material")
    joint_remark = synonym("remarks")
    fluid_service = synonym("line_service")
...
class NDTRecord(Base, CreatedOnlyMixin, ReprMixin):
    ...
    inspector_stamp = Column(String(100), index=True)
    inspector_id = synonym("inspector_stamp")
...
class PipeSupport(Base, CreatedOnlyMixin, ReprMixin):
    ...
    location_desc = synonym("location_description")
    status = Column(String(30), default="Pending", index=True)
    installed_date = Column(Date)
    inspector = Column(String(100))
    remarks = Column(Text)
    ...
    support_subtype = Column(String(80))
    type_2 = synonym("support_subtype")
...
class TestPackage(Base, CreatedOnlyMixin, ReprMixin):
    ...
    install_location = Column(String(20), index=True)
    dia_inch = Column(Float)
    dia_inch_total = synonym("dia_inch")
    inch_metre = Column(Float)
    inch_meter = synonym("inch_metre")
    line_check_finished = Column(Boolean, default=False)
    linecheck_finished = synonym("line_check_finished")
    line_check_result = Column(String(40))
    linecheck_result = synonym("line_check_result")
    line_check_result_date = Column(Date)
    linecheck_result_date = synonym("line_check_result_date")
    line_check_subcontractor = Column(String(150))
    linecheck_subcontractor = synonym("line_check_subcontractor")
...
class WeldReportDraft(Base, ReprMixin):
    ...
    welder_stencil_number = Column(String(100))
    welder_id = synonym("welder_stencil_number")
    welder_name = Column(String(150))
    ...
    rt_report_number = Column(String(100))
    rt_no = synonym("rt_report_number")
    rt_result = Column(String(30))
    pt_report_number = Column(String(100))
    pt_no = synonym("pt_report_number")
    pt_result = Column(String(30))
    ut_report_number = Column(String(100))
    ut_no = synonym("ut_report_number")
...
class ProjectAction(Base, ReprMixin):
    ...
    performed_by_1 = Column(String(100))
    action_by_1 = synonym("performed_by_1")
    performed_by_2 = Column(String(100))
    action_by_2 = synonym("performed_by_2")
...
class Document(Base, TimestampMixin, ReprMixin):
    ...
    _repr_fields = ("id", "document_number", "revision")

    id = Column(Integer, primary_key=True, autoincrement=True)
    project_id = Column(Integer, ForeignKey("projects.id", ondelete="CASCADE"), nullable=False, index=True)
    area_id = Column(Integer, ForeignKey("areas.id", ondelete="SET NULL"), nullable=True)
    document_number = Column(String(100), unique=True, nullable=False, index=True)
    doc_number = synonym("document_number")
    document_type = Column(String(50), nullable=False, index=True)
    doc_type = synonym("document_type")
    ...
    area_name = Column(String(100), index=True)
    document_class = Column(String(50), index=True)
    doc_class = synonym("document_class")
    document_index = Column(String(50))
    doc_index = synonym("document_index")
    service = Column(String(80))
    sheet_number = Column(String(50))
    size_nps = Column(String(50))
    ...
    document_no = synonym("document_number")
...
class TechnicalQuery(Base, TimestampMixin, ReprMixin):
    ...
    description = Column(Text)
    related_document_number = Column(String(200))
    related_document = synonym("related_document_number")
...
class WorkFront(Base, TimestampMixin, ReprMixin):
    ...
    target_quantity = Column(Float, default=0)
    target_qty = synonym("target_quantity")
    actual_quantity = Column(Float, default=0)
    actual_qty = synonym("actual_quantity")
...
class Welder(Base, CreatedOnlyMixin, ReprMixin):
    ...
    qualified_p_number = Column(String(50))
    qualified_material_p_no = synonym("qualified_p_number")
    f_number = Column(String(20))
...
class BlowingRecord(Base, CreatedOnlyMixin, ReprMixin):
    ...
    pressure_barg = Column(Float)
    pressure_bar = synonym("pressure_barg")
...
class BoxUpRecord(Base, CreatedOnlyMixin, ReprMixin):
    ...
    line_number = Column(String(100), index=True)
    manway_flange_number = Column(String(100))
    manway_flange_no = synonym("manway_flange_number")
